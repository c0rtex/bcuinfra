<cfcomponent   >
    <cffunction   access="public" name="processContentRules" output="false" returntype="void"  hint="process content rules" >
	<!-- pass arguments -->
	...
	<cfparam name="arguments.responseStruct.mqc_report_sep_chart" default="N">
	<cfif arguments.subset_id neq 80 and arguments.subset_id neq 83 and arguments.subset_id neq 63>
	<cfif (arguments.responseStruct.mqc_renaldisease neq 'Y') AND (arguments.responseStruct.mqc_medicare_enroll_disability_yes neq 'Y') AND ((arguments.responseStruct.mqc_group_insurance_self eq 'Y' OR arguments.responseStruct.mqc_group_insurance_spouse eq 'Y') AND ((arguments.responseStruct.mqc_employees_gt_20_self eq 'Y') OR (arguments.responseStruct.mqc_employees_gt_20_spouse eq 'Y') OR (arguments.responseStruct.mqc_employees_gt_100_self eq 'Y') OR (arguments.responseStruct.mqc_employees_gt_100_spouse eq 'Y') OR (arguments.responseStruct.mqc_employees_dont_know eq 'Y') OR (arguments.responseStruct.mqc_employees_dont_know_spouse eq 'Y')))>
		<cfset arguments.responseStruct.mqc_report_sep_chart = "Y">
	</cfif>
	</cfif>
	...
	<cfif arguments.debugOutput><cfoutput>#CreateODBCDateTime(Now())#: Starting cacheRulePool code="#rule_content.code#" <br /><br /></cfoutput></cfif>
	   ...
	   <cfelse>
		...
		INNER 
		JOIN wp_posts_v3_1 
		ON wp_postmeta_v3_1.post_id = wp_posts_v3_1.ID
		where meta_key = 'post_code'
		and meta_value like '#trimmed_code#'
