...
<cfif (ThisTag.ExecutionMode eq 'Start' And Not ThisTag.HasEndTag) Or ThisTag.ExecutionMode eq 'End'>
	...
	<cfloop index="itemID" list="cfid,cftoken,source_id,prev_id,org_id,partner_id,subset_id,language_id,access_id,client_id,user_id,state_id,test_id">
		<cfif Not ListFind(attributes.blocklist, itemID)>
			...
			<cfif itemVal neq ''>
				...
				<cfelse>
                	<cfset newhiddenField = '<input type="hidden" name="'&itemID&'" value="'&itemVal&'">'>
					<cfset fieldAccum = fieldAccum & newhiddenField>
				</cfif>
			</cfif>
		</cfif>
	</cfloop>
	...
	<cfloop index="extraID" list="#attributes.extralist#" delimiters="&,">
		...
		<cfelse>
			<cfset fieldAccum = fieldAccum & "<input type=""hidden"" name=""#ListGetAt(extraID, 1, '=')#"" value=""#ListGetAt(extraID, 2, '=')#"">">
		</cfif>
	</cfloop>
</cfif>
<cfif ThisTag.ExecutionMode eq 'Start' And Not ThisTag.HasEndTag>
	...
	<cf_link href="/cf/dispatch.cfm#varAccum#" addtoken="true" canonical="#isCanonical#" var="dispatchURL">
...
<cfelseif ThisTag.ExecutionMode eq 'End'>
		...
		<cfelse>
			...
			<cfelse>
				<cfset attributes.onClick = "this.href='#dispatchURL#';">
					<!--- temp change --->
					<cfif dispatchURL contains '26tgt'>
						<cfset attributes.onClick = "">
					</cfif>
			</cfif>
		</cfif>
	...
	<cfelse>
		...
		<cfset ThisTag.GeneratedContent = ThisTag.GeneratedContent & "#fieldAccum#</form>">
	</cfif>
