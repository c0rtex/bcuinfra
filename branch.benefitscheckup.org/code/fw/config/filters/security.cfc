<cfcomponent
	...
	<cffunction name="configure" access="public" returntype="void" output="false" hint="Configures the filter.">
	...
	<cffunction name="filterEvent" access="public" returntype="boolean" output="false" hint="I am invoked by the Mach II framework.">
		...
		<cfargument name="paramArgs" type="struct" required="false" default="#structNew()#" hint="I am the structure containing the parameters specified in the filter invocation in mach-ii.xml." />	
        <cfset var currentRoles = arrayNew(1) />
        <cfif NOT isDefined("session.user")>
        	<cfset session.user = createObject("component", "fw.model.user.user").init()>
            <cfreturn false />
        <cfelse>
        	<cfset currentRoles = session.user.getRoles() />
            
        	<cfif session.user.hasRole("readonly")>
            	<cfreturn true />
            <cfelse>
            	<cfset event.setArg("errorMsg","You do not have permission to access this area. Please login.") />
