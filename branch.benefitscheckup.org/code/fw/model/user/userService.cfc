<cfcomponent output="no" displayname="userservice" hint="I am a user service.">
	...
	<cffunction name="getUser" access="public" returntype="fw.model.user.user" output="false">
		...
		<cfif arguments.user_id neq 0>
            ...
            <cfloop from="1" to="#arrayLen(roles)#" index="i">
				<cfset roles[i] = variables.roleService.getRole(roles[i].getUsertype_id())>
            </cfloop>
            <cfset user.setRoles(roles)>
		<cfelse>
			<cfset user = createObject("component", "fw.model.user.user").init()>
		</cfif>
		<cfreturn user>
	</cffunction>
