...
<cfloop array="#programs#" index="i">
  <cfset shortDesc = i.getShort_desc()>
  <cfset shortDesc = replaceNoCase(shortDesc,"'","''","ALL")>
  ...
  <cfoutput><p>insert into wp_postmeta(post_id,meta_key,meta_value) select @post_id,'program_short_summary','#shortDesc#' from dual where not(@post_id is null);</p></cfoutput>
</cfloop>
