...
--->
	...
	<cfif listFindNoCase("PR", qState.state_id) GT 0 AND language_id EQ "ES">
		...
		<cfset img_back_button_gif = "Back_to_SNAP_button.png">
	...
	<table cellpadding="0" cellspacing="0" border="0" class="snapTable" style="margin-left: 35px">
		<tr>
...
<div id="snapStateText">

<p>If you need help paying for food and you have limited income and resources, you can submit
an application for SNAP. To apply:</p>
<ul>
<li>Click a link below to download an application or apply online (if available)</li>
<li>Fill out the form following all instructions</li>
<li>Submit the online application or mail the printed application to your local office</li>
</ul>

<p></p>

</div>
					<div id="snap_right_col_application">
	<!---Lynna: removed choose language --->
...
<div> 								<table cellpadding="0" cellspacing="0" border="0">
						...
						<cfif session.partner_id neq 0 AND SESSION.partner_id NEQ 77 AND SESSION.partner_id NEQ 118>
							<div style="padding:6px 12px 8px;  margin-bottom:16px; margin-top:4px;" class="noprint">
<cfif forms_online.recordcount>
...
<!--- Lynna: not needed for the applications page
								...
								<cfif forms.recordcount IS 0>
									...
									<cfloop query="forms">
										...
										<tr>
											...
											<cfif session.partner_id neq 0 AND SESSION.partner_id NEQ 77 AND SESSION.partner_id NEQ 118><td class="snapLegend" valign="top"><cfelse><td class="text1fact" valign="top"></cfif><a href="form_redirect.cfm?id=#form_id#&tgtPartner=#session.partner_id#&tgtorg_id=#session.org_id#&tgt=/forms/#string#" class="snapLink" target="#tag_name#">#name#</a>
												<cfif formtype_id IS 4>
...
--->
                        ...
                        <cfif session.partner_id neq 0 AND SESSION.partner_id NEQ 77 AND SESSION.partner_id NEQ 118>
						<div style="padding:12px; width: 284px;" class="noprint">
                        <cfelse>
...
<p>
<cfset tempWrapperText = #rereplaceNoCase(snap_state.description, "^.*?(?=Hotline)", "")#> <!-- replace everything before Hotline -->
<cfset snap_state.description = tempWrapperText>
<cfset tempWrapperText = #rereplaceNoCase(snap_state.description, "^.*?.Central Office of the Administration for the Socio-Economic Development of the Family \(ADSEF\) at:", "")#> <!-- replace everything before and including -->
<cfset snap_state.description = tempWrapperText>
<cfset tempWrapperText = #rereplaceNoCase(snap_state.description, " to see if you can.*", "")#>
<cfset snap_state.description = tempWrapperText>
<cfset tempWrapperText = #rereplaceNoCase(snap_state.description, ", if you have questions.*$", "")#>
<cfset snap_state.description = tempWrapperText>
<cfset tempWrapperText = #rereplaceNoCase(snap_state.description, "if you have questions.*$", "")#>
<cfset snap_state.description = tempWrapperText>
<cfset tempWrapperText = #rereplaceNoCase(snap_state.description, "How can this program.*$", "")#>
<!---<cfset tempWrapperText = #rereplaceNoCase(snap_state.description, "\..*$", "")#> --->
<cfset snap_state.description = tempWrapperText>
<cfset tempWrapperText = #rereplaceNoCase(snap_state.description, "\.$", "")#>
<cfset snap_state.description = tempWrapperText>


<cfif trim(snap_state.description) IS NOT "Hotline at">
Call your State Hotline to find out where to send the application<strong>: #rereplaceNoCase(snap_state.description, "Hotline at", "")#.</strong>
</cfif>
<!--- Lynna: replace everything up to the word Hotline. If not replaced, the whole description from the SNAP map state-spefic page will show --->
</p>

					</div>
					<!-- Lynna Cekova END snap_right_col_application --> 
					
					<!--- Lynna Cekova #snap_state.description# ---> <!---</td>---> 
		...
		<tr class="noprint">
			<td colspan="4" style="padding-bottom:8px;">
				<table cellpadding="0" cellspacing="0" width="100%" style="margin-left: 35px">
						<tr>
							<td><span>
<br/>
								<a href="/getsnap" class="snapLink">
								<!---<cf_displayImage code="#img_back_button_gif#" hspace="0" vspace="0" border="0"> --->
<img src="/wp-content/uploads/2014/06/Back_to_SNAP_button.png" hspace="0" vspace="0" border="0"/>

</a>
								</span></td>
							<cfif session.partner_id neq 22><td align="right" style="padding-right: 75px"><div><a href="##" onClick="printpage();" class="snapLink">
								<!--- <cf_displayImage code="#img_print_this_page_gif#" hspace="0" vspace="0" border="0"> --->

<br/>
<img src="/wp-content/uploads/2014/06/Print_button.png" hspace="0" vspace="0" border="0"/>
								</a></div></td></cfif>
						</tr>
					</table>
			</td>
