	...
	return {
		...
		link: function(scope, element, attrs, ctrl) {
			...
			function formatter(value) {
				...
				return prefix + currencySym + ' 0';//moneyMask.apply(valueToFormat);
			}
			...
			function parser(val) {
				...
				var dSA = value.split(decimalDelimiter);

				switch (dSA.length) {
					case 1:
						moneyMask = maskFactory(0);
						decimals = 0;
						break;
					default:
						decimals = dSA[dSA.length-1].length==0? 1 : dSA[dSA.length-1].length;
						moneyMask = maskFactory(decimals);
						if (dSA[dSA.length-1].length==0) value = value + '0';
						break;
				}
