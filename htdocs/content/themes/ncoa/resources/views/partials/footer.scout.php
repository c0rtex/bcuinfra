@include('partials.footer-alternate')
<div class="container footer-intro">
    <div class="row">
        <div class="col-sm-12">
            <div class="site-logo">
                <a href="http://ncoa.org" title="NCOA" rel="home">
                    <img width="177" height="60" src="/content/uploads/2016/09/ncoa-logo-177x60.png" class="header-logo" alt="NCOA"  sizes="(max-width: 177px) 100vw, 177px">
                </a>
            </div>

            <p>Improving the health and economic security of 10 million older adults by 2020</p>
            <hr class="infinity-ruler-large">
        </div>
    </div><!-- end row -->
</div>
<div class="container footer-intro">
    <div class="row social-media-bar hidden-xs">
        <div class="col-sm-12">
            <div class="footer-social-media-tray">
                <p>Connect with us on:</p>
                <a class="footer-facebook-icon" href="http://www.facebook.com/NCOAging">&nbsp;</a>
                <a class="footer-twitter-icon" href="http://www.twitter.com/NCOAging">&nbsp;</a>
                <a class="footer-linkedin-icon" href="http://www.linkedin.com/company/216832">&nbsp;</a>
                <a class="footer-group-icon" href="http://www.ncoacrossroads.org/">&nbsp;</a>
                <a class="footer-youtube-icon" href="http://www.youtube.com/ncoaging">&nbsp;</a>
            </div>
            <div class="footer-enews-tray">
                <p>Sign Up for Enews</p>
                <a class="footer-enews-icon" href="/about-ncoa/sign-up/"></a>
            </div>

            <div class="footer-donate-tray">
                <a class="btn btn-purple" href="/get-involved/give-to-ncoa/donate/">Donate</a>
            </div>
            <div class="clearfix"></div>
        </div>
    </div>
</div><!-- end container -->
<div class="full-width footer-bottom-wrapper ">
    <div class="footer-aux-site-network container">
        <div class="site-footer-inner row">
            <div class="col-md-12 col-lg-4">
                <p>Other NCOA Sites</p>
                <div class="menu-footer-network-of-sites-container"><ul id="menu-footer-network-of-sites" class=""><li id="menu-item-366" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-366"><a href="http://www.benefitscheckup.org">BenefitsCheckUp®</a></li>
<li id="menu-item-367" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-367"><a href="http://www.economiccheckup.org">EconomicCheckUp®</a></li>
<li id="menu-item-368" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-368"><a href="http://www.mymedicarematters.org">My Medicare Matters®</a></li>
</ul></div>            </div>
            <div class="trust-icon-container col-md-12 col-lg-8">
                <a href="http://www.charitynavigator.org/index.cfm?bay=search.summary&amp;orgid=4174#.VXHZRM9Viko" target="_blank" class="trust-icon"><img src="http://ncoa.reingolddev.com/wp-content/themes/ncoa2015/images/trust-icon-charity-navigator.png" alt="Charity Navigator"></a>
                <a href="http://give.org/charity-reviews/national/elderly/national-council-on-aging-in-arlington-va-3133" target="_blank" class="trust-icon"><img src="http://ncoa.reingolddev.com/wp-content/themes/ncoa2015/images/trust-icon-bbb.png" alt="Better Business Bureau Accredited Charity"></a>
                <a href="http://greatnonprofits.org/org/national-council-on-aging" target="_blank" class="trust-icon"><img src="http://ncoa.reingolddev.com/wp-content/themes/ncoa2015/images/trust-icon-great-nonprofits.png" alt="2014 Top Rated Non Profit"></a>
                <a href="http://www.opm.gov/combined-federal-campaign/" target="_blank" class="trust-icon"><img src="http://ncoa.reingolddev.com/wp-content/themes/ncoa2015/images/trust-icon-cfc.png" alt="Combined Federal Campaign Approved Charity"></a>
                <a href="https://www.charitywatch.org/ratings-and-metrics/national-council-on-aging/503" target="_blank" class="trust-icon"><img src="http://ncoa.reingolddev.com/wp-content/themes/ncoa2015/images/trust-icon-charity-watch.png" title="Charity Watch" alt="Charity Watch Approved Charity"></a>
                <a target="_blank" class="trust-icon"><img src="http://ncoa.reingolddev.com/wp-content/themes/ncoa2015/images/trust-icon-ima-award.png" title="IMA Award" alt="Interactive Media Award Winner Best in Class 2015"></a>

            </div>
        </div>
    </div>




<div class=" social-media-bar visible-xs-block container">
    <div class="row">
        <div class="col-sm-12">
            <div class="footer-social-media-tray">
                <p>Connect with us on:</p>

<a class="footer-facebook-icon" href="http://www.facebook.com/NCOAging"></a>

<a class="footer-twitter-icon" href="http://www.twitter.com/NCOAging"></a>

<a class="footer-linkedin-icon" href="http://www.linkedin.com/company/216832"></a>

<a class="footer-group-icon" href="http://www.ncoacrossroads.org/"></a>

<a class="footer-youtube-icon" href="http://www.youtube.com/ncoaging"></a>
            </div>
                            <div class="footer-enews-tray">
                    <p>Sign Up for Enews</p>
                    <a class="footer-enews-icon" href="/about-ncoa/sign-up/"></a>
                </div>

<div class="footer-donate-tray-mobile"><a class="btn btn-purple" href="/get-involved/give-to-ncoa/donate/">Donate</a></div>
        </div>
    </div>
</div>

<div class=" site-footer-address-container container">
    <div class="row">
        <div class="site-footer-address col-sm-12 text-center">
            251 18th Street South | Suite 500 | Arlington | VA | 22202 | <a class="ncoa-phone-number" href="tel:571-527-3900">571-527-3900</a>        </div>
    </div>
</div>
</div><!-- close .footer-bottom-wrapper -->
