...
function help(i) {
   var h = open(i, 'Help', 
    "width=400,height=300,status=yes,toolbar=no,menubar=no,scrollbars=yes,resizable=yes");  
} 
...
function linkToUs(i) {
   var h = open(i, 'LinkToUs', 
   "width=600,height=200,status=yes,toolbar=no,menubar=no,scrollbars=yes,resizable=yes");
} 
...
function validatezip(field) {
	...
	if (!ok) {
		alert('The zip code must be all numbers.');
		field.focus();
		field.select();
	} else {
		if (field.value.length != 0 && field.value.length != 5) {
			ok = false;
			alert('The zip code must be 5 digits in length.');
			field.focus();
			field.select();
		}
	}
