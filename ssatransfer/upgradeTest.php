...
foreach($result as $row)
{
	...
	if($row['encryption_flag'] == 1)//MH-decrypts encrypted SSN
	{
		...
		echo $answers[$fieldname]." was encrypted ".$row['response']."<br>";
	}
	...
	echo $fieldname." is ".$row['response']."<br>";
}

//$answers['CT_LIVE_GRT'] = 28;
$answers['CT_CLNT_COSSN'] = '743-98-0029';
$answers['CT_NONCL_COSSN'] = '743-98-0030';
...
//$answers['CT_CLNT_MAIL_ADDR_ST_GRT'] = 28;
...
$answers['BE_THRD_PHONE'] = $answers['BE_THRD_PHNAREA'].$answers['BE_THRD_PHNEXCH'].$answers['BE_THRD_PHNNUM'];
...
if(QA)
{
	$ch = curl_init("https://secureval.ssa.gov/i1020/Ee001Submit.do"); //qa
}
else
{
...
foreach($ApplicantCT->pages as $page)
{
	...
	foreach($page as $key => $value)
	{
		...
		echo "POST URL: ".$tempurl."<br><br>";
		curl_setopt($ch, CURLOPT_URL,$tempurl);	
		}
		if ($key == 'CT_LIVE_GRT')
		{
		echo "POST CT_LIVE_GRT: ".$value."<br><br>";
	...
	print_r($parts);
	echo "<b>TITLE: </b>".$parts[0]."<br><br>";
	
	if($parts[0] == "Print The Re-entry Number")
	{
		preg_match_all('/(<span class="uef-reEntryNumber">)([^<]*)(<\/span>)/', $feedback, $matches);
		$reentry = trim($matches[0][1]); //reentry number removing leading and trailing whitespace
		print_r($matches);
		echo "<b>RE-ENTRY NUM: </b>".$reentry."|<br><br>";
	...
	if($parts[0] == "Warning System Shutdown")
	{
	...
	{		
		...
		if (QA) {		
		//redirect('https://bcuqa.benefitscheckup.org/confirmation.cfm?cfid='.$_GET['CFID'].'&cftoken'.$_GET['CFTOKEN'], 5);
		//redirect("https://".$_SERVER['SERVER_NAME']."/confirmation.cfm?cfid=".$_GET["CFID"].'&cftoken'.$_GET["CFTOKEN"], 5);
		}
		else {
		//redirect("https://".$_SERVER['SERVER_NAME']."/confirmation.cfm?cfid=".$_GET["CFID"].'&cftoken'.$_GET["CFTOKEN"], 5);
		//redirect('https://www.benefitscheckup.org/confirmation.cfm?cfid='.$_GET['CFID'].'&cftoken'.$_GET['CFTOKEN'], 5);
		}
...
//redirect('../confirmation.cfm?cfid='.$_GET['CFID'].'&cftoken'.$_GET['CFTOKEN'], 5);
...
function get_status($title)
{
	switch ($title)
	{
		case "Successful Submission - Print Or Save Your Receipt | Step 4 of 4 | Extra Help With Medicare Prescription Drug Plan Costs | Social Security":
			$status = 1;
			break;
