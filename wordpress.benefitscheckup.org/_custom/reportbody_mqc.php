...
";
				...
				if ($content_code == 'mqc_report_benefitsadvisor_confirm' || $content_code == 'mqc_report_benefitsadvisor_summary' || $content_code == 'mqc_report_benefitsadvisor_challenge' || $content_code == 'mqc_report_benefitsadvisor_overwhelmed' ||  $content_code == 'mqc_report_benefitsadvisor_confident' ||  $content_code == 'mqc_report_benefitsadvisor_planning' ||  $content_code == 'mqc_report_searchplans') {
					...
					$desc3  = str_replace('<span class="learnmore"></span>',' <a href="/medicarequickcheck-transition/?state='.$state_lower.'&'.$reportkey.'">Learn More</a>.', $desc2);
					echo "<li>" .$desc3. "</li>";
	...
	<section id="savings" class="well">  	
  		<div class="body-header-wrapper"><h2><i class="icon-ok-sign "></i> Rx Savings Calculator </h2>			
         <div class="body-header" style="z-index: 999;">
		...
		<div class="alert alert-info">
              ...
              <div >
			<h4>Answer these two questions:</h4>
		     		<iframe class="well" name="picwellframe" scrolling="auto" frameborder="0" width="510" height="375" src="/_custom/loadpicwell.php?session_id='.$screeningID."-".$_GET['request_id']."&zipcode=".$_SESSION['zipcode'].'">
				
				</iframe>
			<h4><a  class="btn btn-large" href="/_custom/loadpicwell.php?session_id='.$screeningID."-".$_GET['request_id']."&zipcode=".$_SESSION['zipcode'].'" target="picwellframe">
			Calculate Savings <i class="icon-chevron-down" ></i> </a> </h4>
			<p>It is likely that you could save this much or more by switching to a different Medicare Part D plan in 2014. To learn more about how we estimate these savings, click <a href="/picwell-estimated-potential-savings/"  >here</a>. If these savings are important to you, we recommend that you take the next step to compare plans and find a better one for you using one of the options we'."'".'ve recommended.</p>
			<p>Here are <a href="http://www.mymedicarematters.org/about-medicare/types-of-coverage/part-d/cost/coping-with-the-coverage-gap/"  target="new">other tips</a> to consider when looking to save money on prescription drugs.</p>
              </div>
...
//print_r($query_contentBCU);
			...
			echo '<p>You should apply for the following benefit programs which can help you pay for your healthcare and prescription drug costs.</p>';
			}
			foreach ($Program_array as $i => $row){
...
<div class="span4">
	   ...
	   <li class="active"><a href="#summary"><i class="icon-bar-chart "></i> Summary </a></li>
          <li class=""><a href="#recs"><i class="icon-user-md "></i> Recommended Actions </a></li>
           <?php if ($showCalculator == 1) { echo '<li class=""><a href="#savings"><i class="icon-ok-sign "></i> Rx Savings Calculator </a></li>'; } ?>
