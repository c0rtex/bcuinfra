...
return true;
   ...
   function logESIRecs ($recsArray, $screeningId) { //called from reportbody_v2.php for ESI
...
logScreeningContent ($screeningId, $recsArray[$row]["postid"], $recsArray[$row]["post_code"]);
    ...
    function getDisplayTextbyID($DisplayId){
		...
		if ($rowCount == 0) {
		$displayText = '<h4 class="Alert-Heading">No Content Found for DisplayID:</h4>'.$DisplayId;
		return $displayText;
		}
		else {	
	...
	//$defTitle = "Definition";
	//$startDef = '<a title="'.$defTitle.'" data-content="'.$defContent.'" html="true"  data-html="true" data-placement="top" data-toggle="popover" href="#" data-original-title="'.$defTitle.'">';
	//$endDef = "</a>"; 
...
,
		...
		$parts = explode('|', $matches[0]);
			...
			$printLink =  '<a title="'.$cleanedLinkTitle.'"  data-html="true" data-content="'.$defContent.'"  data-placement="top" data-toggle="popover" href="#" data-original-title="'.$cleanedLinkTitle.'">'.$cleanedLinkTitle.'</a>';
 			return $printLink;
