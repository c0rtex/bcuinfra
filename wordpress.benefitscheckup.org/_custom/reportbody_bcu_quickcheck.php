...
foreach ($Program_array2 as $i => $row){
	...
	//echo $row["rule_text"]." | ".$row["program_id"]."| ".$esi_zip." | ".$county." | ".$includeprogrambyzip." | ".$includeprogrambycounty." <br> ";	
...
<p>
Want to dig deeper? Take <a href="/esi-questions/">My CheckUp</a> to get a customized report for your individual situation.
</p>
<?php
        ...
        <div class="">
			<div action="" method="get" id="commentForm" class="well cmxform">
				<?php
					...
					// SSI 11712
					$links_bcu_report_supplemental_security = array();
					foreach ($Program_array as $i => $row)
					{
						...
						if(is_numeric($searchIndex)){ 
							...
							$links_bcu_report_supplemental_security[] = $row["program_id"];
							//echo $row["program_id"];
						} 
					}
					
					//print_r($links_bcu_report_supplemental_security);
					unset($i);
					unset($row);
					unset($searchIndex);
					unset($value);
					...
					$link_up = 0;
					$reachout = 0;
					$links_bcu_report_federal_lifeline = array();
					foreach ($Program_array as $i => $row)
					{
						$value = $row["program_code"];
						...
						$money_recs["esi_report_federal_lifeline"]["show"] = 1;
							...
							$links_bcu_report_federal_lifeline[] = $row["program_id"];
						} 
					...
					//print_r($links_bcu_report_federal_lifeline);
					unset($i);
					unset($row);
					unset($searchIndex);
					unset($searchIndex2a);
...
$money_recs["esi_report_food"]["show"] = 1;
							$links_bcu_report_food[] = $row["program_id"];
						} 
...
$money_recs["esi_report_budgeting"]["show"] = 1;
					...
					$links_bcu_report_pension_assitance = array();
					$links_bcu_report_pension_assitance[] = 2081;
...
*/
							...
							echo "<div>";
							if ($title != 'ESI QuickCheck Report') { echo "<h4>" .$title. "</h4>";}
							echo "<p>" .$desc. "<BR />";
        ...
        <div class="">
        	<div action="" method="get" id="commentForm" class="well cmxform">
                <?php
					...
					$links_bcu_report_renter_assistance = array();
					...
					foreach ($Program_array as $i => $row)
					{
						...
						if(is_numeric($searchIndex)){ 
							...
							$links_bcu_report_renter_assistance[] = $row["program_id"];
						} 
					...
					$links_bcu_report_tax_assistance = array();
					foreach ($Program_array as $i => $row)
					{
						...
						if( ( $row["programcategory_id"] == 5) && ($esi_propertytax_receive == 0) && (! is_numeric($searchIndex)) ){ 
							...
							$links_bcu_report_tax_assistance[] = $row["program_id"];
						} 
					...
					//print_r($links_bcu_report_tax_asssitance);
					unset($i);
					unset($row);
					unset($searchIndex);
					unset($value);
					...
					foreach ($Program_array as $i => $row)
					{
						...
						if(is_numeric($searchIndex)){ 
							...
							$links_bcu_report_weatherization = array();
							$links_bcu_report_weatherization[] = $row["program_id"];
						} 
					}
					unset($i);
					unset($row);
...
//echo $esi_liheap_receive;
						...
						if((is_numeric($searchIndex)) && ($esi_liheap_receive == 0)){ 
							...
							$links_bcu_report_utility_assitance = array();
							$links_bcu_report_utility_assitance[] = $row["program_id"];
						} 
					}
					unset($i);
					unset($row);
					...
					if ($req1 == 1 &&  $req2 == 1 &&  $req3 == 1){$housing_recs["esi_report_foreclosure_1"]["show"] = 1;}	// post id 2837
...
//print_r( $Program_array);
					...
					//Medicaid 11710
					$links_bcu_report_medicaid = array();
					foreach ($Program_array as $i => $row)
					{
						$value = $row["program_code"];
						...
						if(is_numeric($searchIndex) || is_numeric($searchIndex_IL)){ 
							...
							$links_bcu_report_medicaid[] = $row["program_id"];
						} 
					...
					//print_r($links_bcu_report_medicaid);
					unset($i);
					unset($row);
					unset($searchIndex);
					unset($value);
					...
					foreach ($Program_array as $i => $row)
					{
						...
						if(is_numeric($searchIndex)){ 
							...
							$links_bcu_report_extra_help = array();
							$links_bcu_report_extra_help[] = $row["program_id"];
						} 
					}
					unset($i);
					unset($row);
					...
					//health_fd_msp_qmb
					//health_fd_msp_slmb
					$links_bcu_report_medicare_savings = array();
					foreach ($Program_array as $i => $row)
					{
						$value = $row["program_code"];
						...
						if($cat_id == 9){ 
							...
							$links_bcu_report_medicare_savings[] = $row["program_id"];
							//echo "msp:".$value.$cat_id."<br>";
						} 
					}
					unset($i);
       	...
       	<div class="">
         	<div action="" method="get" id="commentForm" class="well cmxform">
                <?php
					...
					$AFrow_index = searchForId('esi_retire_legal_concerns', $AF_array);
					if((isset($AF_array[$AFrow_index]["option_code"]) && ($AF_array[$AFrow_index]["option_code"] == 'n'))){
					$links_bcu_report_legal_services = array();
					foreach ($Program_array as $i => $row)
					{
						$value = $row["program_code"];
						//echo $value;
						...
						if(is_numeric($searchIndex)){ 
							...
							$links_bcu_report_legal_services[] = $row["program_id"];
						} 
					}
					}
					unset ($req1);
...
<div class="span4">
   	<div class="well sidebar-nav " style="z-index: 999; position: static; width:220px; top: 191px;">
        ...
        <ol class="nav nav-list">
            <li id="money_li" class="active" class=""><a href="#money"><i class="icon-money "></i> Money Mgmt &#38; Budgeting </a></li>
            ...
            <li id="employment_li" class=""><a href="#employment"><i class="icon-certificate "></i> Employment &#38; Training </a></li>
...
here: <a href="http://www.basiceconomicsecurity.org" target="_blank">www.basiceconomicsecurity.org</a>. 
    	...
    	<div align="center" class="clearfix"><br><br><a target="_blank" href="http://www.wowonline.org"><img border="0" alt="Wider Opportunities for Women" title="Wider Opportunities for Women" src="/cf/img/partners/economicsecurity/logo_wow.png"></a>
      	</div>
