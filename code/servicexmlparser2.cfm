...
<cf_screenPrintedReport pdfname="#session.cftoken#-#session.cfid#-ha" fileformat="pdf" coverpage="true" toc="false"   pgbreaks="false" >
<cfoutput>#session.prg_list# #session.cfid# #session.cftoken#
	...
	<cfif isdefined('resultset')>


	<cfset temp = #QuerySetCell(resultset, "screening_id", session.screening_id, 1)#>
	<cfset temp = #QuerySetCell(resultset, "partner_id", request.partner_id, 1)#>
	<cfset temp = #QuerySetCell(resultset, "subset_id", current_subset_id, 1)#>
	<cfset temp = #QuerySetCell(resultset, "client_id", request.client_id, 1)#>
	<cfset temp = #QuerySetCell(resultset, "report_url", screeningresults.report_url, 1)#>
	<cfset temp = #QuerySetCell(resultset, "program_list", session.newlist, 1)#>
	<cfset temp = QuerySetCell(resultset, "screening_status", "Success", 1)>
	<cfset temp = QuerySetCell(resultset, "reentry_number", "", 1)>
	<cfset temp = #QuerySetCell(resultset, "date_time", now(), 1)#>
...
<cfdump var="#application.prgvars#">
</cfif>
</cfloop>

<cfoutput><br>#session.programtitlelist#</br></cfoutput>