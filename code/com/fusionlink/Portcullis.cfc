<cfcomponent output="false">
	...
	<cfset variables.instance.checkReferer = false/> 								<!---For form variables only--->
	...
	<cfset variables.instance.exceptionFormFields = "">							 	<!---Comma delimited list of fields not to scan--->
	...
	<cffunction name="scan" output="false" access="public" returntype="Void">
		...
		<cfargument name="exceptionFormFields" required="false" type="String"/> 		<!---Comma delimited list of fields not to scan--->
		<cfset var object2 = StructNew()/>
		<cfset var result = StructNew()/>
		<cfset var item= ""/>
		<cfset var itemname= ""/>
		<cfset var exFF= variables.instance.exceptionFormFields/>
		...
		<cfif structkeyexists(arguments,"exceptionFormFields") and len(arguments.exceptionFormFields)>
			<cfset exFF = exFF & "," & arguments.exceptionFormFields/>
		</cfif>
	...
	<cffunction name="isBlocked" output="false" access="public" returntype="Any">
		...
		<cfif variables.instance.checkReferer eq true and isSafeReferer() eq false>
			<cfset blocked = true/>
 		<cfelse>
			<cfquery dbtype="query" name="find">
			SELECT blocked 
