...
Part of the ActivSpell package by ActivSoftware
-->
<html>
<head>
...
<cfif Left(Server.ColdFusion.ProductVersion,1) gte 6>
	<cfif not StructKeyExists(server,"activspell_class")>
		...
		<cfset ldr = CreateObject("java", "java.net.URLClassLoader").newInstance(urlArray,getClass().getClassLoader())>
		<cfoutput></head><body></cfoutput>
		<cfset clazz=ldr.loadClass("com.cfdev.activspell2.SpellChecker")>
...
<!--- com.cfdev.activspell2.SpellChecker see application.cfm for request definitions --->
<cfscript>
...
function done() {
 ...
 if(typeof(parent.opener.aeApplets) != 'undefined') {
	var oRef = '<cfoutput>#jsvar#</cfoutput>';
	oRef = oRef.replace(/aeApplets\[([0-9]+)(\].*)/, "$1");
	parent.opener.aeApplets[oRef].setHtmlContent(string);
 } else {
...
function cancel() {
	parent.opener.ActivSpell.argsIndex = -1;
	parent.close();
}
	...
	<table border="0" width="425" height="200" cellpadding="2" cellspacing="0">
		...
		<tr>
			...
			<td colspan="3" align="right">
				...
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				<input type="button" value="Cancel" width="90" style="width:90" onClick="cancel()">
				</nobr>
			</td>
		</tr>
	</table>
