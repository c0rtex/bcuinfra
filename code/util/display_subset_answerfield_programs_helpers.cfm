...
<cfloop query="questionnaire">
	...
	<tr bgcolor=#CCCCCC>
		...
		<cfloop query="field_programs">
		<cfoutput>program id: #program_id#  </cfoutput>		
		
		
		<cfquery name="parent_programs"  datasource="#session.datasrc#" >
		select * from program p, program_parent pp		
		where 
		p.program_id = pp.program_id and p.program_id = #program_id#

	

		</cfquery>
		<cfloop query="parent_programs"> 
			<cfoutput>helper id: #parent_programs.parent_program_id#</cfoutput>
		</cfloop>
