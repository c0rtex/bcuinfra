...
SELECT
count(distinct(screening_content.screening_id)) as Referral_Count

FROM
screening
INNER JOIN screening_content ON screening.screening_id = screening_content.screening_id
where content_code like 'mqc_report_benefitsadvisor%'
and screening.end_datetime is not null
and screening_content.screening_id >= 6418338
and screening_content.screening_id not in 
(
...
where answerfield_id = 7154 and option_id = 1359

)
</cfquery>
<cfdump var="#results10#"><br><br>
...
<cfquery name="results10"  datasource="#APPLICATION.dbSrc#">				 
SELECT
distinct(screening_content.screening_id),
screening_content.content_code,
screening.end_datetime
FROM
screening
INNER JOIN screening_content ON screening.screening_id = screening_content.screening_id
where content_code like 'mqc_report_benefitsadvisor%'
...
order by screening.end_datetime desc
</cfquery>
