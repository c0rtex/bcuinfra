...
$lang = 'EN';
// Set Program ID
if (isset($_GET['program_id']) and ($_GET['program_id'] != '')) {
...
// Set zip
if (isset($_GET['zip']) and ($_GET['zip'] != '')) {
    $zip = $_GET['zip'];
}
// Set state
...
// Set county
if (isset($_GET['county']) and ($_GET['county'] != '')) {
    $county = $_GET['county'];
}
// Set city
...
$passNumber = 0;
// Set passCounty
if (isset($county) and ($county != '0')) {
    $passCounty = $county;
} else {
    $passCounty = '';
}
// Set passState
if (isset($_GET['state_id']) and ($_GET['state_id'] != '')) {
...
// Set passZip
if (isset($zip) and strcmp($zip, '0') and strcmp($zip, '00000')) {
    $passZip = $zip;
} else {
    $passZip = '';
}
...
//shadow ID
$shadowID = 0;
if (isset($_SESSION['shadowID'])) {
    $shadowID = $_SESSION['shadowID'];
}
...
';
    ...
    if (isset($screeningID)) {
        echo '            <li><a data-toggle="modal" role="button" href="/mqc-results?screeningID=' . $screeningID . '&shadowID=' . $shadowID . '"><i class="icon-circle-arrow-left "></i> Back to Your Report </a></li>';
    }
    echo '
        </ol>		
    </div>	  
