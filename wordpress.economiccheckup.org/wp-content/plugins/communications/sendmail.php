...
if(isset($_POST['emailbody']))
  ...
  if($_POST['sendto'] == 'subscribers')
    {
    $sql = "SELECT `id`,`email` FROM `".$wpdb->prefix."comms` WHERE `subscribed` = '1' and `activated` = '1'";
    }
      ...
      $sql = "SELECT DISTINCT ".$wpdb->prefix."comms.id, ".$wpdb->prefix."comms.email FROM `".$wpdb->prefix."comms`,`".$wpdb->prefix."comms_user_groups` WHERE ".$wpdb->prefix."comms.id = ".$wpdb->prefix."comms_user_groups.userid AND `".$wpdb->prefix."comms.activated='1' AND ".$wpdb->prefix."comms_user_groups.groupid IN (".$group_sql.")";
      }
      else
        {
        $sql = "SELECT `id`,`email` FROM `".$wpdb->prefix."comms` WHERE `activated`='1'";
        }
  ...
  */
  ...
  if($_SESSION['checksentmail'] == '')
    {
    $inc = 0;
    $wpdb->query("INSERT INTO `".$wpdb->prefix."comms_message_log` (`subject`, `message`, `type`, `datetime`) VALUES ('".$_POST['subject']."', '".$_POST['emailbody']."', '0', NOW( ));");
    ...
    $mailer->Body = stripslashes($_POST['emailbody']);
    $mailer->AltBody= stripslashes($_POST['emailbody']);   
    ...
    foreach($list as $email)
      ...
      if(!$mailer->Send())
		{
   		echo 'Message was not sent to '.$email['email'].' because:<br />';
   		echo 'Mailer error: '.$mail->ErrorInfo.'<br /><br />';
		}      
      $mailer->ClearAddresses();
      $wpdb->query("INSERT INTO `".$wpdb->prefix."comms_email_log` (`email` , `commsid` , `messageid` , `bounced` , `errorcode` , `datetime` ) VALUES ('".$email['email']."', '".$email['id']."', '".$msgid[0]['id']."', '0', '', NOW( ));");
