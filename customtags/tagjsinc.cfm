...
function init(sspinc, sspnw, sspa) {
	var theform;
	if (document.formThird) {
		...
		if (sspinc > -1) {
			startpos = 0;
			matchpos = 0;
			endpos = document.formThird["income_earned_General"].length;
			while (startpos < endpos && sspinc >= document.formThird["income_earned_General"][startpos].value) {
				matchpos = startpos;
				startpos++;
			}
			document.formThird["income_earned_General"].selectedIndex = matchpos;
			listIncomeEarned.populate();
			showIncomeEarned();
			startpos = 0;
			matchpos = 0;
			endpos = document.formThird["s_income_earned"].length;
			while (startpos < endpos && sspinc >= document.formThird["s_income_earned"][startpos].value) {
				matchpos = startpos;
				startpos++;
			}
			document.formThird["s_income_earned"].selectedIndex = matchpos;
		}
		
		if (sspnw > -1) {
			startpos = 0;
			matchpos = 0;
			endpos = document.formThird["income_other_nw_General"].length;
			while (startpos < endpos && sspnw >= document.formThird["income_other_nw_General"][startpos].value) {
				matchpos = startpos;
				startpos++;
			}
			document.formThird["income_other_nw_General"].selectedIndex = matchpos;
			listIncomeOtherNW.populate();
			showIncomeOtherNW();
			startpos = 0;
			matchpos = 0;
			endpos = document.formThird["s_income_other_nw"].length;
			while (startpos < endpos && sspnw >= document.formThird["s_income_other_nw"][startpos].value) {
				matchpos = startpos;
				startpos++;
			}
			document.formThird["s_income_other_nw"].selectedIndex = matchpos;
		}
		
		if (sspa > -1) {
			startpos = 2;
			matchpos = 2;
			modpos = document.formThird["s_asset_other_a"].length;
			endpos = modpos + 2;
			while (startpos < endpos && sspa >= document.formThird["s_asset_other_a"][startpos % modpos].value) {
				matchpos = startpos % modpos;
				startpos++;
				if (matchpos == modpos - 1) {
					startpos++;
				}
			}
			document.formThird["s_asset_other_a"].selectedIndex = matchpos;
		}
	}
}
