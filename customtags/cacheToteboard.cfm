...
<cfif not IsDefined('application.bcuToteboard')>
	...
	<!--- Since I've arbitrarily chosen 6 am as the refresh time, we set the next refresh date by adding 20 hours to the current time --->
	<cfset refreshDate = DateAdd('h', 18, Now())>
	<cfset application.bcuToteboard.refreshDateTime = CreateDateTime(Year(refreshDate), Month(refreshDate), Day(refreshDate), 6, 0, 0)>
...
<cfif Evaluate("application.bcuToteboard.#attributes.field#") eq ''>
	...
	where s.subset_id not in (41,53)
		...
		<cfcase value="dollars">
			...
			<cfset application.bcuToteboard.dollars = whdlr.s + 300000000>
		</cfcase>
	</cfswitch>
</cfif>
