...
<cfif ep.RecordCount>
    ...
    <cfif session.partner_id EQ 79>
    	<cfset outstr = outstr & "<div class=""entry_point"" style=""line-height:1.3em;"">">
    <cfelse>
		<cfset outstr = outstr & "<div class=""entry_point"">">
    </cfif>
	<cfif not attributes.reduced and session.partner_id NEQ 79>
		<cfset outstr = outstr & "<br />">
	</cfif>
	<cfset outstr = outstr & "<span class=""epName""><span class='strong'>#ep.name#</span></span><br />">
	...
	<cfif ep.url neq ''>
		...
		<cfelse>
            ...
            <cfif (isdefined("session.mprogram") AND session.mprogram NEQ "")>
            	<cfset outstr = outstr & "<a href=""#ep.url#"" target=""_blank"">Website</a>">
	...
	<cfif attributes.google>
		...
		<cfif trim(street) IS NOT "">
			<cfset outstr = outstr & '<a href="http://maps.google.com/maps?f=q&source=s_q&hl=en&geocode=&q=#address#&ie=UTF8&z=16&iwloc=addr" target="map">View Google Map</a><br />'>
		</cfif>
	</cfif>
	<cfset outstr = outstr & "<BR /></div>">	
