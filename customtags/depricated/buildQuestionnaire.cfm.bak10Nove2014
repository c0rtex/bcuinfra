...
<cfelse>
	...
	<cf_loopPoolStruct struct="session.screening" sortBy="qsort" index="code">
		<cfif not (code eq "food_security" AND session.partner_id NEQ 0) and not (code eq "fit_last_name_2" AND session.partner_id NEQ 76) and not (code eq "fit_agency_id_2" AND session.partner_id nEQ 76) and not (code eq "fit_counseling_session_type" AND session.partner_id nEQ 76) and not (code eq 'learn' and not session.partnerLearnFlag) and not (code eq 'race' and not session.partnerRaceFlag) and not (code eq 'hispanic' and not session.partnerHispanicFlag) and not (session.partner_id eq 27 and session.subset_id eq 51 and code eq 'fit_counseling_session_type_2' )>
			<cfif (code NEQ "fit_last_name_2" OR code NEQ "fit_agency_id_2" or code NEQ "fit_counseling_session_type") and  (code neq 'in_home_care_obligations' or session.partner_id eq 80) and   (code neq 'in_home_care' or session.partner_id eq 80)  and  (code neq 'chicago_agency_screening' or session.partner_id eq 81) and  (code neq 'pl_agency_screening' or session.partner_id eq 81 or session.partner_id eq 61 or session.partner_id eq 63 or session.partner_id eq 68 or session.partner_id eq 102 or session.partner_id eq 85 or session.partner_id eq 75 or session.partner_id eq 115)  AND (code neq 'hr_healthplan_screening' or session.partner_id eq 47) and (code neq 'int_chase_direct_deposit' or (session.partner_id eq 0 and session.org_id lt 1)) and (code neq 'reverse_mortgage' or (session.partner_id eq 0 and session.org_id lt 1))>
