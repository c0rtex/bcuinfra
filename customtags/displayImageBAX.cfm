...
<cfelse>
	<cfset outstr = '<img '>
	<cfset outstr = outstr & "src=""#application.transferType#://#application.serverPath#/#iobj.file#"" ">
	<cfif attributes.alt eq ''>
		<cf_displayText group="imgalt" code="imgalt_#attributes.code#" alt="{{!!nada!!}}" var="diAltText">
		<cfif diAltText neq '{{!!nada!!}}'>
			<cfset outstr = outstr & "alt=""#HTMLEditFormat(diAltText)#"" ">
		<cfelse>
			<cfset outstr = outstr & "alt="""" ">
		</cfif>
	<cfelse>
		<cfset outstr = outstr & "alt=""#HTMLEditFormat(attributes.alt)#"" ">
	</cfif>
