...
<!-- Only insert entry point heading if ep info available - first store as substring and insert into output if ep success -->
	...
	<!---<table cellpadding="0" cellspacing="0" border="0" <!---<cfif CGI.SCRIPT_NAME CONTAINS "frmdetails" AND URL.county EQ 'NEW YORK'>width="55%"<cfelse>width="100%"</cfif>--->>
		<cfif attributes.suppressText IS false>
			<tr>
				<td colspan="2" class="text1" style="height: 30px;" width="100%">
...
<cfif session.partner_id NEQ 79 and session.partner_id neq 14 and session.partner_id neq 0>
    <cfif attributes.source neq "ec" or methodSucceeded >
        <cfset outstr = "<strong class='factSheetH2'>For further information about this program:</strong>" & strSubHeading & outstr & "</div></div>">
    </cfif>
<cfelse>
	<cfset outstr = "<br>" & strSubHeading & outstr & "</div></div>">
</cfif>
<cfif attributes.debugvar neq ''>
