...
function initBarGraph(){
	curBar = new StackedBar('.cur-graph');
	retBar = new StackedBar('.ret-graph');

	curBar.height = 425;
	retBar.height = 425;
}

function refreshBarGraph(num){
	curBar.refresh();
	retBar.overall = curBar.getOverflow();
	retBar.refresh();
}
...
function initHowMuch(){
	...
	refreshBarGraph(getHowMuchMaxVal());
...
function setTargetMonthlyConsumption(delta){
	...
	retBar.updateBar('graph-target_monthly_consumption', Math.round(target));
	refreshBarGraph(getHowMuchMaxVal());
	return target;
}
...
function updateMortgageSpending(){
	...
	curBar.updateBar('graph-monthly_mortgage_payment', mm);
...
function getMonthlySpending(){
	...
	*/
	...
	var spending = roundTo(totalEarn, 10) - roundTo(mort, 10);
	spending -= roundTo(contributions, 10);
	spending -= roundTo(ssaPayroll, 10);
	spending -= roundTo(extra, 10);
	spending -= roundTo(incomeTax, 10);
...
function recalcAll(){
	...
	// DS: Need to update medical expenses too
...
function updateTargetTaxes(){
	var retSpending = shebangGetValue('target_monthly_consumption');
	var retirementSSATaxRate = getTargetSSARetirementTaxRate(toNumber(retSpending));
	var extraMedical = shebangGetValue('medical_expenses_dollar');
	...
	retBar.updateBar('graph-post_retirement_monthly_income_tax_amount', Math.round(monthlyRetirementIncomeTax));
...
function updateTargetIncome(){
	...
	// DS: Rounding to nearest 10 to match bar graph
	// shebangUpdatePageValue('target_ret_income', addCommas(roundTo(tamt, 100)));
	shebangUpdatePageValue('target_ret_income', addCommas(roundTo(tamt, 10)));
...
function updateMedicalExpenses () {
	var medicalExpenses = isMarried ? 300 : 150;
	retBar.updateBar('graph-medical_expenses_dollar', medicalExpenses);
	shebangSaveValue('medical_expenses_dollar', medicalExpenses);
	retBar.refresh();
...
function updateIncomeTax(){
	...
	curBar.updateBar('graph-pre_retirement_monthly_income_tax_amount', cmit);
...
function updateMonthlySpending(){
	...
	curBar.updateBar('graph-current_monthly_consumption', Math.round(spending));
...
function updateMonthlyCombinedIncome(){
	...
	shebangUpdatePageValue('current_monthly_income', addCommas(roundTo(cearn, 10)));
...
function updatePayrollTaxAmt(){
	...
	curBar.updateBar('graph-payroll_tax_amt_combined', payrollTaxAmt);
	refreshBarGraph(getHowMuchMaxVal());
}
...
function updateRetSavings(){
	...
	curBar.updateBar('graph-hh_monthly_retirement_saving_amount', totalSavingsMonth);
	refreshBarGraph(getHowMuchMaxVal());
}
...
function updateExtraExpenses(){
	...
	retBar.updateBar('graph-other_retirement_expenses', val);
	refreshBarGraph(getHowMuchMaxVal());
}