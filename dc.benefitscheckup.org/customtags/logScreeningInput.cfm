...
<cfif #findnocase('s-zip',lsiList)# gt 0 and (session.partner_id eq 81 or session.partner_id eq 63 or session.partner_id eq 61) and #findnocase('o-pl_agency_list',lsiList)# eq 0>
...
<cfif ListFind(lsiDeriveList, 'skipinc')>
	...
	<cfif lsiSkipIncome eq 0>
		...
		<cfloop index="element" list="pri_retire,dividends,ssi,ss_disable,ss_retire,rr_ben,vet_ben,unemploy,tanf,child_support,cash_assist,other_nw,earned">
			<cfloop index="class" list="s,sp,s_sp,hh">
